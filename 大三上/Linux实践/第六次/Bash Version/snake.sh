...
game_start=(
    ...
    '         press   "w,a,s,d" to steer the sanke    '
    '         press    q       quit at any time       '
    '         press    l       change the speed       '
    '                                                 '
...
snake_exit() {  #退出游戏
	i=1;
	rm ./result.txt
	echo "****************Report****************" >>result.txt;
	echo "You have tried ${#scores[@]} times !" >>result.txt;
	bestscore=${array[0]};
	for I in ${!scores[@]};do
       	if [[ ${bestscore} -le ${scores[${I}]} ]];then
        bestscore=${scores[${I}]};
      	fi
		echo "Score of time $i : ${scores[${I}]}" >> result.txt;
		i=$(($i+1));
	done
	echo "The best score is : $bestscore ! " >>result.txt;
	echo "***************Thank you**************" >>result.txt;
    stty echo;  #恢复回显
    tput rmcup; #恢复屏幕
    tput cvvis; #恢复光标
    cat result.txt;
